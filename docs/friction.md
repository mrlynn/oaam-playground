...
**2026-09-18 · The instructions stop most transient memories, not all.** Real use on `aim_live`, with `EXTRACTION_INSTRUCTIONS` set: the user asked about a product the assistant couldn't answer for, then asked again in a later thread. Extraction stored "The user has asked about 'Oracle AI Agent Memory' again after a previous inquiry" and "As of the latest exchange, the question… remains unresolved". The wording forbade what the *assistant* asked and "the state of the conversation", and the extractor read these as facts about the user. It also kept storing what the assistant said about itself ("The assistant does not have access to real-time weather data"). `spikes/custom_instructions.py --compare` adds a probe that repeats an unanswered question and tests a second wording (no questions asked or how often or whether answered, nothing the assistant said about itself, keep the lasting interest). Over three trials: 7 transient-looking memories down to 1, 3 about the assistant down to 0, 66 memories down to 57, and every correction still said what it corrects. The one that got through: "continues to show interest…, having asked about it multiple times". So a prompt can shrink this class of memory, not close it: the case for making durable-only the package default (fix 3) stands, and so does a check that finds what the prompt misses. Also noticed: passing `memory_extraction_custom_instructions` straight to `create_thread` is now deprecated in favour of `MemoryExtractionConfig`.
