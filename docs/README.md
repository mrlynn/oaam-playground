...
Status: **milestone 3, step 1**. There is a read-only thread view on real rows, and the run log schema is installed by the new `memory-inspector` library. The wrapper that fills it, the companion agent, the scrubber and the "why" panel are next ([plan](plans/m3-run-log.md)).
...
| `agent/` | demo config (`aim_demo`), seed/smoke scripts, schema dumper, scripted conversations |
